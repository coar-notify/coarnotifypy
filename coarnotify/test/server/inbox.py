...
def create_app():
    """Create the flask app, pulling config from ``settings.py`` then any supplied local config in environment variable
    ``COARNOTIFY_SETTINGS``.
...
class COARNotifyServiceTestImpl(COARNotifyServiceBinding):
    ...
    def notification_received(self, notification: NotifyPattern) -> COARNotifyReceipt:
        ...
        3. Return a receipt for the notification using the configured response status
           and a location pointing to the file
...
def inbox():
    ...
    This pulls the notification out of the request as JSON, and sends it to the server which will parse it and send it
    on to the service binding implementation
...
def run_server(host=None, port=None, fake_https=False):
    ...
    :param host:
    :param port:
    :param fake_https: if fake_https is True, developer can use https:// to access the server
    :return:
    """
    ...
    if pycharm_debug:
        ...
        pydevd.settrace(
            app.config.get('DEBUG_PYCHARM_SERVER', 'localhost'),
