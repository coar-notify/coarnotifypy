...
class COARNotifyReceipt:
    ...
    Server implementations should construct and return this object with the appropriate properties when implementing the
    :py:meth:`COARNotifyServiceBinding.notification_received` binding
    ...
    def status(self) -> int:
        ...
        Should be one of the constants ``CREATED`` (201) or ``ACCEPTED`` (202)
        """
        return self._status
...
class COARNotifyServiceBinding:
    ...
    That method will receive a :py:class:`NotifyPattern` object, which will be one of the known types and should return
    a :py:class:`COARNotifyReceipt` object with the appropriate status code and location URL
    """
...
class COARNotifyServerError(Exception):
    ...
    :py:meth:`COARNotifyServer.receive` method and return the appropriate HTTP status code and message to the user in
    its standard way.
...
class COARNotifyServer:
    ...
    :py:meth:`receive` method, which will parse the payload and pass it to the
    :py:meth:`COARNotifyServiceBinding.notification_received` method of your service implementation
    ...
    def __init__(self, service_impl: COARNotifyServiceBinding):
        """Construct a new COARNotifyServer with the given service implementation :param service_impl: Your service
        implementation."""
        self._service_impl = service_impl

    def receive(self, raw: Union[dict, str], validate: bool = True) -> COARNotifyReceipt:
        """Receive an incoming notification as JSON, parse and validate (optional) and then pass to the service
        implementation.
